...
class Tracer:
    ...
    def traceFunctionBody(self, func, mark_as_injected = False):
        ...
        if func.name == 'glEGLImageTargetTexture2DOES':
            ...
            print('    EGLClientBuffer buffer;')
            print('    if (target == GL_TEXTURE_EXTERNAL_OES) {')
...
if __name__ == '__main__':
    ...
    with open(file_path, 'w') as f:
        ...
        print()
        print('inline char* inject_timestamp(unsigned char tid, char* dest)')
        print('{')
        print('    uint64_t ts = os::getTimeType(CLOCK_MONOTONIC);')
        print('    BCall *pCall = (BCall*)dest;')
        print('    pCall->funcId = paTimestamp_id;')
        print('    pCall->tid = tid;')
        print('    pCall->errNo = 0;')
        print('    pCall->source = 1;')
        print('    pCall->reserved = 0;')
        print('    dest += sizeof(*pCall);')
        ...
        api.delFunctionByName("paTimestamp")
        tracer.traceApi(api)
        sys.stdout = orig_stdout
